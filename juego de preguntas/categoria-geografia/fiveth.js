...
async function getData() {
    ...
    try {
        ...
        question.innerHTML = data.questions[4].pregunta
        firstOption.innerHTML += data.questions[4].respuesta_correcta
        secondOption.innerHTML += data.questions[4].respuesta_incorrecta1
        thirdOption.innerHTML += data.questions[4].respuesta_incorrecta2
        fourthOption.innerHTML += data.questions[4].respuesta_incorrecta3
    }
    catch (error) {
        console.log(error)
    }
...
next.addEventListener('click', () => {
    ...
    setTimeout(() => {
        window.location.href = "/juego%20de%20preguntas/result.html";
    }, 1500)
});

function verAnswer() {
    const correctOption = firstOption;

    if (secondOption.style.backgroundColor === "rgb(55, 188, 48)") {
